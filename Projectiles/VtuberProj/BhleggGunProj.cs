...
{
    public class BhleggGunProj : KnifeProjectile
    {
            ...
            if (projectile.owner == Main.myPlayer)
            {
                ...
                if (stillInUse)
                {
                    ...
                    if (ShootTimer > ShootTimerMax)
                    {
                        ...
                        int ran = (int)(Main.rand.NextFloat(-20 + (ShootTimerAdd * 6), 20 - (ShootTimerAdd * 6)));
                        float spread = MathHelper.ToRadians(ran)/5;
                        ...
                        Projectile.NewProjectile(InitialPos.X, InitialPos.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ProjectileType<Projectiles.VtuberProj.BhleggProj>(), (int)((float)projectile.damage * 1.5f), projectile.knockBack, player.whoAmI);
                        ShootTimer = 0;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            {
                Offset = new Vector2(player.direction * 13, 0);
            }
            else
            {
                Offset = new Vector2(player.direction * 1, 0);
            }
