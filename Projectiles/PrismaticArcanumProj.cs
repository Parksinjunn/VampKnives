...
{
    public class PrismaticArcanumProj : KnifeProjectile
    {
        int Delay;
        bool HasChanneled;
        float SpeedMult;
        ...
        public override void AI()
        {
            ...
            {
                int num120 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width, projectile.height, 66, 0f, 0f, 100, new Color(Main.DiscoR, Main.DiscoG, Main.DiscoB), 2.5f);
                Dust dust3 = Main.dust[num120];
                dust3.velocity *= 0.1f;
                dust3 = Main.dust[num120];
                dust3.velocity += projectile.velocity * 0.2f;
                Main.dust[num120].position.X = projectile.position.X + (float)(projectile.width / 2) + 4f + (float)Main.rand.Next(-2, 3);
                Main.dust[num120].position.Y = projectile.position.Y + (float)(projectile.height / 2) + (float)Main.rand.Next(-2, 3);
