            ...
            if (projectile.owner == Main.myPlayer)
            {
                ...
                if (stillInUse)
                {
                    ...
                    if (ShootTimer > ShootTimerMax)
                    {
                        ...
                        Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ProjectileType<SpectreProj>(), (int)((float)projectile.damage * 1.5f), projectile.knockBack, player.whoAmI);
