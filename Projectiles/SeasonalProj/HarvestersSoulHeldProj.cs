    ...
    public class HarvestersSoulHeldProj : KnifeProjectile
    {
            ...
            if (projectile.owner == Main.myPlayer)
            {
                ...
                if (stillInUse)
                {
                    ...
                    if (ShootTimer > ShootTimerMax)
                    {
                        ...
                        Projectile.NewProjectile(InitialPos.X, InitialPos.Y, baseSpeed * (float)Math.Sin(offsetAngle), baseSpeed * (float)Math.Cos(offsetAngle), ProjectileType<HarvestersSoulProj>(), 1 + (int)((float)projectile.damage * ((float)ShootTimerAdd /4f)), projectile.knockBack, player.whoAmI);
        ...
        private void PlaySounds()
        {
            ...
            if (projectile.soundDelay <= 0)
            {
                ...
                if (FrameCounter > 1f)
                {
                    Main.PlaySound(SoundID.Item117.WithVolume(0.25f).WithPitchVariance(-1f), projectile.position);
                }
            }
        }
