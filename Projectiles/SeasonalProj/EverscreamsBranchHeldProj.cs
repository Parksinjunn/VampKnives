            ...
            if (projectile.owner == Main.myPlayer)
            {
                ...
                if (stillInUse)
                {
                    ...
                    if (ShootTimer > ShootTimerMax)
                    {
                        ...
                        Projectile.NewProjectile(InitialPos.X, InitialPos.Y, baseSpeed * 1.6f * (float)Math.Sin(offsetAngle), baseSpeed * 1.6f * (float)Math.Cos(offsetAngle), ProjectileType<EverscreamsBranchProj>(), 1 + (int)((float)projectile.damage * ((float)ShootTimerAdd /4f)), projectile.knockBack, player.whoAmI);
