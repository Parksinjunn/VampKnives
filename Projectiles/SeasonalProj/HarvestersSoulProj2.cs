    ...
    public class HarvestersSoulProj2 : KnifeProjectile
    {
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            if (EffectTimer > (TrailLength - (1 + (((TrailLength - 1) / 4) * 3))))
                spriteBatch.Draw(mod.GetTexture("Projectiles/SeasonalProj/HarvestersSoulProj2"), drawPos, null, color, projectile.rotation, drawOrigin, projectile.scale, SpriteEffects.None, 0f);
            if(EffectTimer > (TrailLength - (1 + (((TrailLength - 1) / 4) * 2))))
                spriteBatch.Draw(mod.GetTexture("Projectiles/SeasonalProj/HarvestersSoulProj2"), drawPos2, null, color2, projectile.rotation, drawOrigin, projectile.scale, SpriteEffects.None, 0f);
            if(EffectTimer > (TrailLength - (1 + (((TrailLength - 1) / 4) * 1))))
                spriteBatch.Draw(mod.GetTexture("Projectiles/SeasonalProj/HarvestersSoulProj2"), drawPos3, null, color3, projectile.rotation, drawOrigin, projectile.scale, SpriteEffects.None, 0f);
            if(EffectTimer > (TrailLength - 1))
                spriteBatch.Draw(mod.GetTexture("Projectiles/SeasonalProj/HarvestersSoulProj2"), drawPos4, null, color4, projectile.rotation, drawOrigin, projectile.scale, SpriteEffects.None, 0f);
            return false;
        }
    }
}