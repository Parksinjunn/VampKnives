...
namespace VampKnives
{
    public class VampPlayer : ModPlayer
    {
        ...
        public bool NyanTransform;
        public bool MouseWingEquipped;
        public bool MouseTransform;
        public bool MouseMilkBuff;

        public bool SpectreGlovesOn;
        public bool BookReset;

        internal Item UpgradeItem;
        //internal List<Item> RitualItemsList;
        //internal int RitualListSize;
        ...
        public override void ResetEffects()
        {
            ...
            Bandaged = false;
            VeiWingsEquipped = MouseWingEquipped = false;
            VeiTransform = NyanTransform = MouseTransform = false;
            MouseMilkBuff = false;
        ...
        public override void Initialize()
        {
            ...
            //for(int i = 0; i < RitualItemsList.Count; i+=2)
            //{
            //    RitualItemsList[i] = new Item();
            //}
        }
        public void SyncBPToServer()
        {
            ModPacket packet = mod.GetPacket();
            packet.Write(VampKnives.SyncBloodPoints); 
            packet.Write(BloodPoints);
            packet.Write(player.whoAmI);
            packet.Send();
        }
        public override void clientClone(ModPlayer clientClone)
        {
            VampPlayer clone = clientClone as VampPlayer;
            ...
            clone.NyanTransform = this.NyanTransform;
            clone.MouseTransform = this.MouseTransform;
        }

        public override void SendClientChanges(ModPlayer clientPlayer)
        {
            VampPlayer clone = clientPlayer as VampPlayer;
            if (HoodIsVisible != clone.HoodIsVisible)
            {
                ModPacket packet = mod.GetPacket();
                packet.Write(VampKnives.HoodServerRecieve);
            ...
            else if(Transform != clone.Transform)
            {
                ...
                packet2.Write(VampKnives.BatTransformRecieve);
                packet2.Write(Transform);
                packet2.Write(HasTabletEquipped);
                packet2.Write(Main.myPlayer);
                packet2.Send();
            }
            else if(BloodPoints != clone.BloodPoints)
            {
                SyncBPToServer();
            }
            else if(VeiTransform != clone.VeiTransform)
            {
                ...
                packet3.Write(VampKnives.VeiTransormRecieve);
                packet3.Write(VeiTransform);
                packet3.Write(Main.myPlayer);
                packet3.Send();
            }
            else if(NyanTransform != clone.NyanTransform)
            {
                ModPacket packet3 = mod.GetPacket();
                packet3.Write(VampKnives.NyanTransformRecieve);
                packet3.Write(NyanTransform);
                packet3.Write(Main.myPlayer);
                packet3.Send();
            }
            else if(MouseTransform != clone.MouseTransform)
            {
                ModPacket packet = mod.GetPacket();
                packet.Write(VampKnives.MouseTransformRecieve);
            ...
            else if(SupportArmorKeyPressed != clone.SupportArmorKeyPressed)
            {
                for (int x = 0; x < Main.ActivePlayersCount; x++)
                {
                    ...
                    {
                        ModPacket packet2 = mod.GetPacket();
                        packet2.Write(VampKnives.SupportArmorRecieve);
        ...
        public override void SetupStartInventory(IList<Item> items, bool mediumcoreDeath)
        {
            ...
            {
                if (items[x].type == ModContent.ItemType<Items.Misc.StartupBook>())
                {
                    if (BookCount == 0)
                    {
                        BookCount++;
        ...
        public override void UpdateVanityAccessories()
        {
            for (int n = 13; n < 18 + player.extraAccessorySlots; n++)
            {
                ...
                if (item.type == ModContent.ItemType<Items.VtuberItems.SuccubusHeartCorset>())
                {
                    VeiTransform = true;
                }
                if (item.type == ModContent.ItemType<Items.VtuberItems.GamerHeadphones>())
                {
                    NyanTransform = true;
                }
                if (item.type == ModContent.ItemType<Items.VtuberItems.DemonLoli>())
                {
                    MouseTransform = true;
                }
            }
        ...
        int identifier;
        public bool SendMiningPackage;
        public bool MiningPackageLeft;
        public bool MiningPackageRight;
        public bool MiningPackageTop;
        ...
        //                KillBlocksSend.Write(VampireWorld.RitualOfSouls[g]);
        //            }
        //            KillBlocksSend.Send();
        //        }        
        //    }
        ...
        public override void FrameEffects()
        {
            ...
            {
                player.head = mod.GetEquipSlot("VeiTransformHead", EquipType.Head);
                player.body = mod.GetEquipSlot("VeiTransformBody", EquipType.Body);
                player.legs = mod.GetEquipSlot("VeiTransformLegs", EquipType.Legs);
            }
            if(NyanTransform && !VeiTransform && !HasTabletEquipped && !MouseTransform)
            {
                player.head = mod.GetEquipSlot("NyanTransformHead", EquipType.Head);
                player.body = mod.GetEquipSlot("NyanTransformBody", EquipType.Body);
                player.legs = mod.GetEquipSlot("NyanTransformLegs", EquipType.Legs);
            }
            if(MouseTransform && !VeiTransform && !NyanTransform && !HasTabletEquipped)
            {
                player.head = mod.GetEquipSlot("MouseTransformHead", EquipType.Head);
                player.body = mod.GetEquipSlot("MouseTransformBody", EquipType.Body);
                player.legs = mod.GetEquipSlot("MouseTransformLegs", EquipType.Legs);
            }
            //if(VeiWingsEquipped)
            //{
            //    if (player.bodyFrameCounter == 0 && (player.velocity.X < 0 || player.velocity.X > 0 || player.velocity.Y < 0 || player.velocity.Y > 0))
        ...
        public static readonly PlayerLayer VeiWings = new PlayerLayer("VampKnives", "VeiWings", PlayerLayer.MiscEffectsBack, delegate (PlayerDrawInfo drawInfo)
        {
            ...
            VampPlayer modPlayer = drawPlayer.GetModPlayer<VampPlayer>();
            ...
            if (modPlayer.VeiWingsEquipped)
            {
                ...
                int drawY = (int)(drawInfo.position.Y + drawPlayer.height / 2f - (Main.screenPosition.Y-202));
            ...
            else if(modPlayer.MouseWingEquipped)
            {
                Texture2D texture = mod.GetTexture("Items/VtuberItems/MouseWingsAnim");
                int frameSize = texture.Height/10;
                int drawX;
                if (drawPlayer.direction == 1)
                {
                    drawX = (int)(drawInfo.position.X + drawPlayer.width / 2f - (Main.screenPosition.X + 2));
                }
                else
                {
                    drawX = (int)(drawInfo.position.X + drawPlayer.width / 2f - (Main.screenPosition.X - 2));
                }
                int drawY = (int)(drawInfo.position.Y + drawPlayer.height / 2f - (Main.screenPosition.Y - 132));
                if (drawPlayer.mount.Active == true)
                    drawY += 20;
                DrawData data = new DrawData(texture, new Vector2(drawX, drawY), new Rectangle(0, frameSize * modPlayer.VeiWingFrame, texture.Width, frameSize), Color.White, 0f, new Vector2(texture.Width / 2f, texture.Height / 2f), 1f, SpriteEffects.None, 0);
                Main.playerDrawData.Add(data);
        ...
        {
            if (VeiTransform || NyanTransform || MouseTransform)
            {
                player.fullRotation = player.velocity.Y * (float)player.direction * 0.03f;
                player.fullRotation = Utils.Clamp(player.fullRotation, -0.4f, 0.4f);
                if(player.HasBuff(ModContent.BuffType<Buffs.VTuberBuffs.MeatBicycleBuff>()) && player.velocity.Y <= 0f)
                {
                ...
                bool RightSide()
                {
                    if (((Main.screenWidth / 2) - Main.MouseScreen.X) < 0)
                    {
                        return true;
                    }
                    else
                ...
                if(RightSide())
                {
                    RotationFactor = (Main.MouseWorld - player.position).ToRotation() * -1;
                }
                else
                {
                    RotationFactor = (player.position - Main.MouseWorld).ToRotation();
                }
                if (player.direction == -1)
                {
                    player.headRotation = RotationFactor;
                    player.headRotation = Utils.Clamp(player.headRotation, -0.2f, 0.2f);
                }
                else
                {
                    player.headRotation = RotationFactor * -1;
                    player.headRotation = Utils.Clamp(player.headRotation, -0.2f, 0.2f);
                }
                //if (ZoneExample)
                //{
        ...
        public override void ModifyDrawLayers(List<PlayerLayer> layers)
        {
            ...
            if ((VeiTransform || NyanTransform || MouseTransform) && !HasTabletEquipped)
            {
                layers.Remove(PlayerLayer.Skin);
                //layers.Remove(PlayerLayer.Face);
                //layers.Remove(PlayerLayer.Arms);
